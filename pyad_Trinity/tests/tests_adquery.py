...
class TestADQuery(ADTestCase):
    def setUp(self):
        self.ad_query = pyad_Trinity.adquery.ADQuery()
    ...
    def test_dne_single_result(self):
        ...
        self.ad_query.execute_query(where_clause=query)
        self.assertRaises(
            pyad_Trinity.pyadexceptions.invalidResults, self.ad_query.get_single_result
        )
    ...
    def test_single_single_result(self):
        query = "cn = '%s'" % self.KNOWN_EXISTS_USER
        self.ad_query.execute_query(
            attributes=("cn", "distinguishedname"), where_clause=query
        )
        self.assertEqual(
